...
# Extract Vso_values
Vso_param = config.get("sweep", {}).get("Vs", [-10, 5, 10])
Vso_values = []
while Vso_param[0] <= Vso_param[2]:
    Vso_values.append(round(Vso_param[0], 10))
    Vso_param[0] += Vso_param[1]
# Vso_values =equiv np.arange(start=-10, stop=10, step=5)
...
# Iterate, for each pair of elements of Vc_values X Vso_values a input file is created
i = 0
